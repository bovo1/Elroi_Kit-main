...
class AesEncryption(object):
    ...
    def make_fire(self, input_data, out_filename, key=main_key, _type="lic"):
        '''
        데이터 암호화 후 파일로 추출
        '''
        try:
            if _type == "lic":
                ...
                data = self._to_bytes(input_data)
            elif _type == "model":
                data = input_data
            elif _type == "mat":
                input_data = str(input_data)
                ...
                raise Exception(f"{_type} does not supported")
            ...
            aes_key, mac_key = self._keys(salt, key)
    ...
    def make_water(self, in_filename, key=main_key, _type="lic"):
        '''
        암호화된 데이터 복호화
        '''
        try:
            ...
            if _type == "lic":
                json_file = plaintext.decode('utf-8')
    ...
    # def make_fire(self, key, input_data, out_filename):
    #     '''
    #     데이터 암호화 후 파일로 추출
    #     '''
    #     try:
    ...
    # def make_water(self, key, in_filename):
    #     '''
    #     암호화된 데이터 복호화
    #     '''
    #     try:
